...
test('readFocusedTextTarget 默认允许未知应用在同一前台窗口内尝试粘贴', async () => {
  const result = await readFocusedTextTarget({
    startFocusInfo: {
      appInfo: { app_metadata: { hwnd: '700' } },
      elementInfo: { focused: true, editable: true },
    },
    readTextTarget: async () => ({
      success: false,
      source: 'none',
    ...
    readWindowTree: async () => ({
      ...
      process_name: 'Code',
      foreground_hwnd: '700',
      window_title: 'typeless - Visual Studio Code',
      class_names: ['Chrome_WidgetWin_1'],
    }),
  });

  assert.equal(result.success, true);
  assert.equal(result.source, 'foreground_window');
  assert.equal(result.reason, 'same_foreground_window');
  assert.equal(result.foregroundHwnd, '700');
  assert.deepEqual(result.matchedSignals, ['same_foreground_hwnd']);
});
