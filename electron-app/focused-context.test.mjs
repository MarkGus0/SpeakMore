...
test('readFocusedTextTarget 在 UIA 和 caret 都失败时接受 VS Code app_compat 目标', async () => {
  const result = await readFocusedTextTarget({
    startFocusInfo: {
  ...
  assert.equal(result.success, true);
  assert.equal(result.source, 'app_compat');
  assert.equal(result.reason, 'app_compat_match');
  assert.equal(result.foregroundHwnd, '700');
  assert.equal(result.appFamily, 'vscode');
  assert.deepEqual(result.matchedSignals, [
    'process:vscode',
    'same_foreground_hwnd',
    'class:Chrome_WidgetWin_1',
