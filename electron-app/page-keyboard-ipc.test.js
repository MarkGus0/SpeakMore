...
test('registerKeyboardIpcHandlers 在 macOS 通过平台能力粘贴且不启动 Windows 自动学习', async () => {
  const ipcMain = createFakeIpcMain();
  const calls = [];
  ...
  registerKeyboardIpcHandlers({
    ...
    readFocusedInfo: async () => ({}),
    textObservationManager: {
      start: async () => calls.push('observer-start'),
    },
  ...
  assert.deepEqual(calls, [['paste', 'hello', { startFocusInfo }]]);
});
