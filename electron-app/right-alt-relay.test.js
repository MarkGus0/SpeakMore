...
test('RightAlt 超短点按在 keyup 前就应先发出 keydown', () => {
  ...
  assert.deepEqual(emissions[1], [
    ...
    { keyName: 'RightCommand', isKeydown: false },
  ]);
});
...
test('RightAlt 按下后再按 RightCommand，应发出自由提问需要的当前键态', () => {
  const { relay, emissions } = createHarness();

  relay.handlePayload({ key: 'RightAlt', isKeydown: true });
  relay.handlePayload({ key: 'RightCommand', isKeydown: true });
...
test('没有 RightAlt 时，单独 RightCommand 不应发任何键态', () => {
  const { relay, emissions } = createHarness();
...
test('释放 RightCommand 后，应在短延迟后恢复到 RightAlt 单键态', () => {
  const { relay, emissions, timers } = createHarness();

  relay.handlePayload({ key: 'RightAlt', isKeydown: true });
  relay.handlePayload({ key: 'RightCommand', isKeydown: true });
  relay.handlePayload({ key: 'RightCommand', isKeydown: false });

  assert.deepEqual(emissions[2], [{ keyName: 'RightCommand', isKeydown: false }]);
...
test('释放 RightAlt 时会同步释放 RightCommand 虚拟键', () => {
  const { relay, emissions } = createHarness();
